	...
	public void createSimpleContainer() {
		...
		for (KieContainerResource kieContainerResource : kieContainers) {
			if (kieContainerResource.getContainerId().equals(RULES_CONTAINER)) {
				System.out
						.println("Found rules container. No new container will be created.");
				container = kieContainerResource;
			}
		}
		if (container == null) {
			System.out.print("Container " + RULES_CONTAINER
					+ " not found. Creating it...");
			System.out
					.println("(make sure you installed hello_rule-1.0.jar in your repository)...");
